...
import com.example.finalproject.Controller.StaffController;
...
public class StaffControllerTest {
    ...
    @Test
    void getAllStaffsTest() throws Exception {
        Mockito.when(staffService.getStaff()).thenReturn(staffs);
        mockMvc.perform(get("/api/v1/staff/get-allstaff"))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$", Matchers.hasSize(1)))
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].name").value("Mohammed"));
    }
